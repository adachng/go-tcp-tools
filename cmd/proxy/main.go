...
// flag.UintVar(&ret.ListenPort, "l", 0, "Proxy listen port")
...
func main() {
	p := flag.Uint("p", nil, "ABC")
	flag.Parse()

	if p == nil {
		logx.Default().Fatal(p)
	} else {
		logx.Default().Panic(p)
	}

	c := proxy.Config{ListenPort: 8080}
	logx.Default().Infof("%+v\n", c)

	{
		testVar := net.ParseIP("192.168.9.3")
		logx.Default().Infof("testVar = %+v\n", testVar)
	}

	{
		testVar := net.ParseIP("192.168.9.3:1234")
		logx.Default().Infof("testVar = %+v\n", testVar)
	}

	{
		conn, err := net.Dial("tcp", "10.220.197.4:8022")
		if err != nil {
			logx.Default().Fatal(err)
		}

		logx.Default().Notice("conn.RemoteAddr = ", conn.RemoteAddr())
		logx.Default().Notice("conn.LocalAddr = ", conn.LocalAddr())
	}
}
